	...
	return (
		<section className='w-full max-w-60'>
			...
			<div className='relative w-full h-[33px]'>
				<input
					type={type === 'LOGIN' ? 'text' : isActive}
					className='w-full h-full bg-white px-2 pr-6 outline-none text-[14px] placeholder:text-[#AAAAAA]'
					placeholder={placeholder}
					onChange={e => onChange(e.target.value)}
				/>
				<Image
					src={type === 'LOGIN' ? LoginSVG : EyeSVG}
					alt='...'
					...
					onClick={() =>
						type === 'PASSWORD' &&
						setIsActive(isActive === 'password' ? 'text' : 'password')
					}
				/>
			</div>
		</section>
